...
public class ErroresControlador implements ErrorController {
    ...
    private int getErrorCode(HttpServletRequest httpRequest) {
        ...
        if (statusCode == null) {
            statusCode = httpRequest.getAttribute("javax.servlet.error.status_code"); //por si se está usando la versión vieja
        }
        if (statusCode instanceof Integer) {
            return (Integer) statusCode;
        }
        // Maneja el caso en que el atributo es null o no es un Integer
        return -1; // O podes usar otro valor por defecto adecuado
    }
